...
export function MonthView({
  ...
  const normalized = lessons.map((l) => ({
    ...l,
    date: normalizeLessonDate(l.date) || l.date,
  }));
  const byDate = groupLessonsByDate(normalized);
  const today = new Date();
  const monthFrom = format(monthStart, 'yyyy-MM-dd');
  const monthTo = format(monthEnd, 'yyyy-MM-dd');
  const monthLessons = normalized.filter((l) => l.date >= monthFrom && l.date <= monthTo);
  ...
  return (
    <div className="space-y-5">
      ...
      <CalendarLessonList
        ...
        students={students}
        viewer={viewer}
        emptyTitle="Нет занятий в этом месяце"
        emptyDescription="В выбранном месяце занятий нет."
      />
    </div>
  );
}
