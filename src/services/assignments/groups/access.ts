...
export function canViewAssignmentGroup(user: User, group: AssignmentGroup): boolean {
  ...
  if (isGeneralAssignmentGroup(group) && canManageAssignmentGroups(user)) return true;
  ...
  if (can(user, 'assignments:view-own') && isGeneralAssignmentGroup(group)) return true;
  return false;
}

export function canEditAssignmentGroup(user: User, group: AssignmentGroup): boolean {
  if (isGeneralAssignmentGroup(group)) return false;
